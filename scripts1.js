const data = [
    ...
    // Sukurkite masyvą visų žmonių, kurių marškinių dydžiai XS arba S; ir surūšiuokite šį masyvą
    // pagal vardus, A-Z tvarka (alfabetiškai).
    console.log(data.filter((person) => person.shirt_size === "XS" || person.shirt_size === "S").sort((a, b) => (a.first_name > b.first_name ? 1 : -1)))

    // Pakoreguokite trečią pratimą, kad masyve matytųsi tik id, vardas bei marškinių dydis.
    console.log(data.filter((person) => person.shirt_size === "XS" || person.shirt_size === "S").map((person) => ({
        id: person.id,
